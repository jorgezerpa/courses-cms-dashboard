...
export const SectionRow = ({ section }:{ section:any }) => {
  ...
  return (
    <tr>
      <td className="px-4 py-4 text-sm font-medium whitespace-nowrap">
        <div>
          <h2 className="font-medium text-gray-800 dark:text-white ">{ section.name }</h2>
      ...
      <td className="px-4 py-4 text-sm whitespace-nowrap">
        <div>
          <p className="text-gray-500 dark:text-gray-400 max-w-[200px] text-ellipsis overflow-hidden">{section.description}</p>
        </div>
      </td>
          ...
          <div className={`top-0 right-[90%] absolute ${showItemOptions?'block':'hidden'}`}>
              <div id="dropdown" className="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow shadow-gray-500 dark:bg-gray-700">
                <ul className="py-2 px-3 text-sm text-gray-700 dark:text-gray-200" >
                  <li className='text-left' onClick={()=>router.push(`/courses/${router.query.courseId}/sections/${section.id}`)}>
